...
VS Code 装了 Aero 扩展（`aero-lang`），但代码写错时编辑器没提示。

**原因**：语法高亮开箱就有，编译错误要靠扩展连上编译器的语言服务。**解决**：确认扩展版本是 1.2.1 或更新，并在设置里把 `aero.lsp.executablePath` 指向 `aero.exe` 的完整路径。诊断消息的格式是 1.2.1 才修好的——更早的版本即使路径配对，编辑器也会把诊断整个丢掉。
